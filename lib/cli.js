...
function parseArgs (argv, requireFiles) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (arg) {
      ...
      case '--parsing-config': {
        const parsingConfigFile = argv[i + 1]

        if (!parsingConfigFile) {
          throw new Error('Missing parsing config file. Usage: --parsing-config [path/to/vuedoc.config.js]\n')
        }

        // eslint-disable-next-line
        options.parsingConfig = require(parsingConfigFile)
        i++
        continue
      }
      ...
      default: {
        ...
        if (arg.startsWith(ignorePrefix)) {
          ...
          options.parsingConfig.features = options.parsingConfig.features.filter((item) => item !== feature)
          continue
        }
...
const processRawContent = (argv, componentRawContent) => {
  ...
  options.parsingConfig.filecontent = componentRawContent
