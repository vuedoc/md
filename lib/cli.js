...
function parseArgs (argv, requireFiles) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (arg) {
      ...
      case '--stringify': {
        if (!argv[i + 1]) {
          throw new Error('Missing stringify value. Usage: --stringify [boolean]\n')
        }

        const value = argv[i + 1]

        if (value !== 'true' && value !== 'false') {
          throw new Error('Invalid stringify value. Usage: --stringify [boolean]\n')
        }

        options.stringify = value === 'true'
        i++
        continue
      }
