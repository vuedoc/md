...
function renderCommonTags (options, entry) {
  if (entry.since) {
    options.$printlni(Md.bold(options.$tr(I18nKey.since)), entry.since);
  }

  if (entry.version) {
    options.$printlni(Md.bold(options.$tr(I18nKey.version)), entry.version);
  }

  if (entry.author instanceof Array && entry.author.length) {
