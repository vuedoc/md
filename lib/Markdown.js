...
function printAuthors (options, entry) {
...
module.exports.render = (component, options = {}) => {
  ...
  options.wordwrap = options.wordwrap || DEFAULT_WORDWRAP;

  options.$tr = (key) => options.labels[key] || I18nLabel[key];
  options.$wrap = (text, indent = '') => wordwrap(text, {
    indent,
    trim: true,
  ...
  process.nextTick(() => {
    ...
    if (component.author instanceof Array && component.author.length) {
      printAuthors(options, component);
    }
