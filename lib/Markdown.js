...
import { MethodParam } from '@vuedoc/parser/entity/method';
import { KeywordsUtils } from '@vuedoc/parser/utils/KeywordsUtils';
import { Feature, CommonTags, TagAlias, JSDocTag, Type } from '@vuedoc/parser/enum';
import { I18nKey, I18nLabel } from './I18n.js';
...
function printTable(items, fields, keys, options) {
  ...
  options.$println(markdownTable([head, ...body]));
  options.$println();
}
...
function renderDescriptionAndExamples(options, entry) {
  const tags = TagAlias.description.concat([JSDocTag.example]);
  const keywords = KeywordsUtils.extract(entry.keywords, tags, true);

  if (entry.description) {
    keywords.unshift({
      name: JSDocTag.description,
  ...
  keywords.forEach(({ name, description }) => {
    switch (name) {
      case JSDocTag.example:
        options.$println(Md.bold(options.$tr(I18nKey.example)));
        options.$println();
...
function renderMethodBody(options, method, nbsp = '<br/>') {
  renderDescriptionAndExamples(options, method);
  ...
  if (method.params.length) {
    ...
    if (hasDescribedParameters) {
      ...
      const paramItems = method.params.map((param) => {
        let chunk = Md.backtick(MethodParam.toString(param));
  ...
  computed: [
    ...
    TypeField,
    {
      ...
      render(description, computed, options) {
        const tokens = [
          parseValue(description, computed, DescriptionField),
        ].filter((token) => token);
  ...
  events: [
    NameField,
    {
      ...
      render(description, event, options) {
        const tokens = [
          parseValue(description, event, DescriptionField),
        ];
...
export function render(component, options = {}) {
  ...
  options.features = options.features || VuedocParser.SUPPORTED_FEATURES;

  options.$tr = (key) => options.labels[key] || I18nLabel[key];

  options.$wrap = options.wordwrap <= 0
  ...
  const start = () => {
    ...
    if (options.features.includes(Feature.description)) {
      renderDescriptionAndExamples(options, component);
    }
