...
const indentText = (text, level = DEFAULT_INDENT_LEVEL, slug = DEFAULT_INDENT_SLUG) => text.split(/\n/g)
...
function printParams (params, options, title = 'parameters') {
  ...
  params.forEach((param) => {
    ...
    options.$print('-', indentText(param.description, level).trim())
    options.$println()
  })
...
const writer = {
  ...
  props (options, props, title) {
    ...
    props.forEach((prop) => {
      /* eslint-disable-next-line arrow-body-style */
      const customTypeKeyword = prop.keywords.find(({ name, description }) => {
      ...
      /* eslint-disable-next-line arrow-body-style */
      const defaultKeyword = prop.keywords.find(({ name, description }) => {
  ...
  methods (options, methods, title) {
    ...
    methods.forEach((method) => {
      ...
      if (method.return) {
        ...
        options.$println(
          item(bold(method.return.type)), '-', method.return.desc
        )
      }
    })
  },
...
module.exports.render = (component, options = {}) => {
  ...
  options.$print = (...args) => {
    args.forEach((str) => emiter.emit('write', `${str} `))
  }
