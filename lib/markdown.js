...
const { Syntax, DEFAULT_TYPE, UNDEFINED } = require('@vuedoc/parser/lib/Enum')
  ...
  const params = method.params
    ...
    .map(({ name, type, declaration, defaultValue, optional, default: defaultParamValue, repeated }) => {
      ...
      if (defaultValue instanceof Value) {
        paramDefaultValue = defaultValue.type === Syntax.ObjectExpression
          ? JSON.stringify(defaultValue.value)
          : defaultValue.value
      }

      if (!paramType) {
        paramType = paramDefaultValue ? typeof paramDefaultValue : DEFAULT_TYPE
      }
