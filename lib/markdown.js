...
function parseType (type, keywords = []) {
  if (keywords.length) {
    const typeKeyword = keywords.slice(0).reverse().find(({ name }) => name === 'type');

    if (typeKeyword && typeKeyword.description) {
      return typeKeyword.description
    }
  }
...
function printTableColumnValue (value, item, { separator = ', ', valueIsArray = isArray, ...fieldOptions }) {
  return valueIsArray(value, item)
...
const Fields = {
  props: [
    ...
    },
    {
      ...
      highlight: true,
      separator: ' | ',
      valueIsArray: (value, { keywords }) => (keywords.length ? !keywords.some(({ name }) => name === 'type') : value instanceof Array),
      render: (type, { keywords }) => parseType(type, keywords)
    },
    {
      key: 'description',
      header: 'Description',
    ...
    methods.forEach((method) => {
      ...
      if (method.params.length) {
        ...
        if (hasDescribedParameters) {
          ...
          method.params.forEach(({ type, name, description }) => {
            let param = name
            ...
            options.$print(item(bold(backtick(param))))
      ...
      if (method.return.description) {
        ...
        options.$println()
        options.$println(method.return.description.trim())
        options.$println()
      }
    })
  },
