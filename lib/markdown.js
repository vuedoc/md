...
const writer = {
  ...
  props (options, props, title) {
    ...
    props.forEach((prop) => {
      ...
      const defaultKeyword = prop.keywords.find( ({ name }) => name === 'default')
      const defaultValue =  !prop.value.hasOwnProperty('default')
        ? undefined
        : defaultKeyword && defaultKeyword.description
