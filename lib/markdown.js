...
function renderMethodTitle (value, method) {
  const params = method.params
    ...
    .map(({ name, type, declaration, defaultValue, optional, default: defaultParamValue }) => {
      let paramType = type instanceof Array ? type.join(' | ') : type
      let paramDefaultValue = defaultParamValue ? JSON.stringify(defaultParamValue) : null
  ...
  return `${value}(${args}): ${method.return.type}`
}
...
const Fields = {
  ...
  events: [
    ...
    },
    {
      key: 'arguments',
      separator: '<br>',
      render: ({ name, type, declaration, definition }) => {
        const tokens = [
          bold(backtick(name)),
          italic(`(${type || 'any'})`)
        ]
...
function printTable (items, options) {
  ...
  const data = items.map((item) => {
    return options.fields.reduce((accumulator, { key, separator = ', ', ...fieldOptions }) => {
  ...
  options.$println(table([header, ...data]))
  options.$println()
}
  ...
  methods (options, methods, title) {
    ...
    if (!options.table) {
      ...
      methods.forEach((method) => {
        ...
        const syntaxKeyword = method.keywords.find(({ name }) => name === 'syntax')

        if (syntaxKeyword) {
          options.$println(code(syntaxKeyword.description, 'ts'))
        } else {
