...
const writer = {
  props (options, props, title) {
    ...
    props.forEach((prop) => {
      const type = prop.entry[prop.name].type || prop.entry[prop.name] || 'any'
      const nature = prop.entry[prop.name].required ? 'required' : 'optional'
      const twoWay = prop.entry[prop.name].twoWay
      let defaultValue = prop.entry[prop.name].default
